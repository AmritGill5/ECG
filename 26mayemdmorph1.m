...
%normalize_signal = @(x) 2 * (x - min(x)) / (max(x) - min(x)) - 1;
...
% Normalize the signals
y2 = normalize_signal(y);
n_s2 = normalize_signal(n_s);
...
% Display the result
disp(['Mean Squared Error (MSE) using immse: ', num2str(mse_i)]);


% Initialize variables
z = length(y);
imfc = zeros(1, ensemble_count); % Array to store the number of IMFs for each ensemble
imfs = [];
...
for i = 1:ensemble_count
    ...
    % Store the IMFs
    if i == 1
        % Initialize imfs array with the size based on the first EMD decomposition
        imfs = zeros(ensemble_count, size(imf_temp, 1), size(imf_temp, 2));
    end
    
    imfs(i, :, 1:size(imf_temp, 2)) = imf_temp;
    imfc(1, i) = size(imf_temp, 2);
end

% Find the most common number of IMFs
win_imf = mode(imfc);

% Select only those ensembles with the most common number of IMFs
matching_imfs = imfs(imfc == win_imf, :, 1:win_imf);

% Average the selected IMFs across ensembles (1st dimension)
avg_imfs = squeeze(mean(matching_imfs, 1));
z_imf=avg_imfs';
...
se1 = strel('line', 0.38*fs, 0);  % Create a structuring element (line, length 3)
...
y_f3 = normalize_signal(y_f2);

% Calculate Mean Squared Error (MSE) after filtering
mse_out = mean((y2- y_f3).^2);
disp(['Mean Squared Error (MSE) after filtering: ', num2str(mse_out)]);
...
% Plot the original signal and its averaged IMFs
figure;
